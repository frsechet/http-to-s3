...
class HttpToS3 {
  constructor(options = {}) {
    ...
    this.throwFailures = throwFailures || false; // If a pre-configured S3 client is provided, use it
  ...
  async request(url, options = {}, body) {
    ...
    const parsedUrl = parseUrl(url); // Only http/https supported
    ...
    const requestOptions = {
      ...
      // convert null or undefined to blank string
      ...userOptions
    };
    const response = await getHttpStream(requestOptions, body); // To add additional extensibility, we could accept a callback and pass
  ...
  async post(url, options = {}, body) {
    const postOptions = options; // Some object cleansing
