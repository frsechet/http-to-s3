...
describe("request method", () => {
  it("Should successfully stream to S3", async () => {
    ...
    const client = new HttpToS3();
    getHttpStream.mockReturnValueOnce(Promise.resolve({
      statusCode: 200
    }));
  ...
  it("Should throw errors if throwFailures is set", async () => {
    ...
    handleHttpError.mockReturnValueOnce(Promise.resolve(mockErrorResponse)); // We could use Jest's toThrow() expectation, but by manually catching the
    // the error, we can easily test the extra values that are attached to it

    try {
      await client.request("https://example.com"); // this test should never be executed, but will obviously fail human error
...
describe("Post method", () => {
  it("Should set the method correctly", async () => {
    expect.assertions(1);
    const client = new HttpToS3(); // Set the request method to a simple jest fn so we can capture its calls
